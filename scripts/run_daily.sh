...
# -i 防系统闲置睡眠（屏幕可睡，不影响 CDP 自动化）；-s 防系统睡眠——2026-09-05 事故：
# 合盖 Sleep Service 回睡（pmset 特征 Sleep Service Back to Sleep）-i 拦不住，审批窗内
# 隧道被杀断、点确认打到死 URL；-s 在 AC 电源下连合盖也保持系统运行（电池供电时无效，
# 但流水线机器常年在 AC）。-w $$ 盯本脚本 PID，脚本退出即自动撤除断言，不留孤儿。）
