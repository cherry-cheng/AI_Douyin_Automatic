...
const TITLE = '7.5英寸平板手机杀回来了'
const DESCRIPTION = '手机厂商开始嫌弃屏幕不够大了 📱\n\n爆料称荣耀正规划7.5英寸16:10宽屏"平板手机"：骁龙旗舰芯+万级电池+主动散热。几个月前还在卷小屏，转头就冲向大屏——因为AI手机爆发年要来了，分屏和AI助手都需要更大的施展空间。\n\n你的裤兜还塞得下吗？评论区聊聊 👇'
const TOPICS = ['平板手机','荣耀手机','数码科技','科技资讯']
...
async function clearAndFillDouyinBody(description) {
  ...
  for(let attempt=0;attempt<3;attempt+=1){
    ...
    const selected=await js(String.raw`(() => {const editor=document.querySelector('#vp2-douyin-editor');if(!editor)return{ok:false,reason:'douyin editor lost while selecting body'};editor.focus();const selection=window.getSelection(),range=document.createRange();range.selectNodeContents(editor);selection.removeAllRanges();range.collapse(false);selection.addRange(range);const active=document.activeElement;return{ok:active===editor||editor.contains(active),activeTag:active?.tagName||'',activeId:active?.id||''}})()`)
...
await wait(1)
const finalSnap = await snapshotText()
cliLog(finalSnap.slice(0, 1000))
