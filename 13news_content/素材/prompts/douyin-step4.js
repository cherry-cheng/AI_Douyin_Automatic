...
if (entry && (entry.y < 40 || entry.y > entry.vh - 40)) {
  await scroll({ dy: 300 }); await wait(1.2)
  entryPt = await js(String.raw`(() => {
    const vis=el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};
    const want=['添加音乐','选择音乐','配乐','选择配乐'];
    const cands=[...document.querySelectorAll('button,div,span,a')].filter(el=>vis(el)&&want.some(w=>(el.textContent||'').trim()===w));
    let pick=cands.find(el=>getComputedStyle(el).cursor==='pointer');
...
if (entryPt) cliLog('entry: ' + JSON.stringify(entryPt))
if(!entryPt){ cliLog('ℹ️ 没找到配乐入口，跳过配乐') }
else if(entryPt.y<40||entryPt.y>entryPt.vh-40){ cliLog('⚠️ 配乐入口不在视口安全区(y='+entryPt.y+')，先滚动再试') }
else {
  await cdp('Input.dispatchMouseEvent',{type:'mouseMoved',x:entryPt.x,y:entryPt.y})
  await cdp('Input.dispatchMouseEvent',{type:'mousePressed',x:entryPt.x,y:entryPt.y,button:'left',clickCount:1,buttons:1})
  await cdp('Input.dispatchMouseEvent',{type:'mouseReleased',x:entryPt.x,y:entryPt.y,button:'left',clickCount:1,buttons:1})
  await wait(2.5)
  ...
  cliLog(s.replace(/\s+/g,' ').slice(0, 600))
}
