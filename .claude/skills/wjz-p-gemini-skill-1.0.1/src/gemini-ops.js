...
const SELECTORS = {
   ...
   * 生成图片容器内的 <img>
   *
   * Gemini UI 改版后（2026），生成图片不再是 img.image.loaded，
   * 而是渲染在 <generated-image> 自定义元素（.generated-image）内。
   * 新结构优先，旧结构兜底。
   */
  generatedImageImg: [
    'generated-image img',     // 新版：自定义元素内的 img
    '.generated-image img',    // 新版：class 变体
    'img.image.loaded',        // 旧版结构兜底
  ],
  /**
   * "下载完整尺寸"按钮（hover 图片后出现的工具栏按钮）
   *
   * 2026 Gemini 改版后两处失配，导致 fullSize 下载一直 full_size_download_btn_not_found：
   *   1. data-test-id 挂在外层 <gem-icon-button> 自定义元素上，不在 <button> 上
   *      → 旧选择器 button[data-test-id=...] 永远匹配不到
   *   2. 中文 aria-label 变成「下载完整尺寸的图片」
   *      → 旧选择器 [aria-label*="下载图片"] 子串不匹配（中间隔着「完整尺寸的」）
   * 现按真实 DOM 命中，宽松兜底放后面。
   */
  generatedImageDownloadBtn: [
    'button[aria-label*="下载完整尺寸"]',              // 新版中文真实 aria
    'button[aria-label*="Download full" i]',             // 新版英文 aria 推测
    '[data-test-id="download-generated-image-button"]', // data-test-id 在 gem-icon-button 上（不限标签）
    '[data-test-id="download-enhanced-image-button"]',
    'button[aria-label*="下载"]',                       // 中文宽松兜底
    'button[aria-label*="Download" i]',                 // 英文宽松兜底
    'button[data-test-id="download-generated-image-button"]', // 旧版兼容
    'button[aria-label*="下载图片"]',                   // 旧版兼容
  ],
  tempChatBtn: [
...
export function createOps(page) {
  ...
  return {
    ...
    async isModelPro() {
      ...
      // 兼容 pill 显示 "Pro" 或 "3.1 Pro"；排除 Flash/Flash-Lite
      return result.model.includes('pro') && !result.model.includes('flash');
    },
    ...
    async switchToModel(model) {
      const validModels = ['pro', 'quick', 'think'];
      if (!validModels.includes(model)) {
        return { ok: false, error: `unknown_model: ${model}` };
      }

      // 旧版 data-test-id 选择器（UI 改版前的结构，作为兜底）
      const legacySels = {
      ...
      // 3. 优先：按菜单项文本匹配（兼容 2026 改版后的版本化模型名 + hash test-id）
      //    菜单项为 <gem-menu-item>，标签在 .label span 内（如 "3.1 Pro"、"3.6 Flash"、"扩展思考"）
      const textResult = await page.evaluate((targetModel) => {
        const items = Array.from(document.querySelectorAll('gem-menu-item, [data-test-id^="bard-mode-option"]'))
          .filter(el => { const r = el.getBoundingClientRect(); return r.width > 0 && r.height > 0; });
        const labelOf = (el) => (el.querySelector('.label')?.textContent || el.textContent || '').trim();
        let target = null;
        if (targetModel === 'pro') {
          // "3.x Pro"，排除含 flash/lite/think 的项
          target = items.find(el => { const l = labelOf(el); return /pro/i.test(l) && !/flash|lite|扩展思考|think/i.test(l); });
        } else if (targetModel === 'quick') {
          // "3.x Flash"，排除 "Flash-Lite"
          target = items.find(el => { const l = labelOf(el); return /flash/i.test(l) && !/lite/i.test(l); });
        } else if (targetModel === 'think') {
          // "扩展思考" / "Deep Think"
          target = items.find(el => /扩展思考|deep\s*think|thinking/i.test(labelOf(el)));
        }
        if (!target) return { ok: false };
        target.click();
        return { ok: true, matched: labelOf(target).slice(0, 40) };
      }, model);

      // 4. 文本匹配失败 → 旧选择器兜底
      if (!textResult.ok) {
        const selectResult = await op.click(legacySels[model]);
        if (!selectResult.ok) {
          // 关闭菜单，避免菜单遗留影响后续操作
      ...
      console.log(`[ops] switched model: ${previousModel || '?'} → ${model}${textResult.ok ? ` (matched "${textResult.matched}")` : ' (legacy selector)'}`);
    ...
    async getAllImages() {
      return op.query((sels) => {
        const _all = sels.flatMap(s => { try { return [...document.querySelectorAll(s)]; } catch { return []; } });
        const imgs = [...new Set(_all)].filter(el => { const r = el.getBoundingClientRect(); return r.width >= 80 || r.height >= 80; });
        if (!imgs.length) {
    ...
    async getLatestImage() {
      return op.query((sels) => {
        // 收集所有生成图片（新 UI 的 <generated-image> 优先，旧 UI 的 img.image.loaded 兜底）
        const seen = new Set();
        const imgs = [];
        for (const sel of sels) {
          let found;
        ...
        while (container && container !== document.body) {
          ...
          if (cl?.contains('image-container') || cl?.contains('generated-image') || tag === 'generated-image') break;
        ...
        const dlBtn = container
          ? (container.querySelector('mat-icon[fonticon="download"]')
            ...
            || container.querySelector('button[data-test-id^="download"]'))
          : null;
   ...
   async extractImageBase64(url) {
     ...
     if (isBlob) {
       ...
       const canvasResult = await op.query((targetUrl, sels) => {
         const _all = sels.flatMap(s => { try { return [...document.querySelectorAll(s)]; } catch { return []; } });
         const imgs = [...new Set(_all)];
         const img = imgs.find(i => i.src === targetUrl || i.currentSrc === targetUrl);
         if (!img) {
           // 如果精确匹配失败，回退到最后一张
       ...
       }, url, SELECTORS.generatedImageImg);
     ...
     *   2. CDP Browser.setDownloadBehavior 尝试把下载重定向到 config.outputDir（部分环境不生效，由步骤 4 兜底）
     *   3. hover 触发工具栏 → 在目标图所属 .generated-image 容器内点其专属「下载完整尺寸」按钮
     *      （2026 改版修复：旧的全局选择器会失配或命中视口外的按钮；竖图还会让工具栏溢出视口）
     *   4. 文件系统轮询判完成（替代 CDP downloadProgress 事件——后者在 Chrome 151 等版本不触发，
     *      会导致永远 download_timeout），取「点击后新出现、大小已稳定」的文件，移动到 config.outputDir
     *   5. 返回实际保存的文件路径
     *
    ...
    async downloadFullSizeImage({ index, timeout = 30_000 } = {}) {
      ...
      const scrollResult = await op.query((targetIndex, sels) => {
        const seen = new Set();
        const imgs = [];
        for (const sel of sels) {
          let found;
      ...
      }, index, SELECTORS.generatedImageImg);
      ...
      const imgInfo = await op.query((targetIndex, sels) => {
        const seen = new Set();
        const imgs = [];
        for (const sel of sels) {
          let found;
        ...
        const i = targetIndex == null ? imgs.length - 1 : targetIndex;
        const img = imgs[i];
        if (!img) return { ok: false, error: 'no_loaded_images' };
      ...
      }, scrollResult.index, SELECTORS.generatedImageImg);
      ...
      // 3. 记录下载目录的「点击前快照」，用于点击后检测新文件
      //    不依赖 CDP Browser.downloadProgress 事件——该事件在部分 Chrome 版本（如 151）不触发，
      //    会导致完成检测永远 download_timeout。改用文件系统轮询判完成，更可靠。
      const { readdirSync } = await import('node:fs');
      const { homedir } = await import('node:os');
      const watchDirs = Array.from(new Set([downloadDir, pathResolve(homedir(), 'Downloads')]));
      const snapByDir = new Map();
      for (const d of watchDirs) {
        try { snapByDir.set(d, new Set(readdirSync(d))); } catch { /* 目录可能不存在 */ }
      }
      ...
      // 5. 点击"下载完整尺寸"按钮（带重试 + 视口对齐）
      //    关键修复（2026 Gemini 改版后旧逻辑失效）：
      //    a) 不能用全局 op.click(btnSelector) —— 页面上每张生成图都有同款下载按钮，
      //       locate().find() 会命中第一个（往往已滚出视口），点错图。
      //       改为从「目标 img」出发，在其 .generated-image 容器内找它专属的下载按钮。
      //    b) 竖图很高时，前面 scrollIntoView(center) 图片会让顶部工具栏溢出视口
      //       （按钮 getBoundingClientRect().top 为负），点击空坐标无效、下载不触发。
      //       点击前需把按钮本身 scrollIntoView 到视口内（走 Gemini 的内部滚动容器，非 window）。
      const btnSels = SELECTORS.generatedImageDownloadBtn;
      const imgSels = SELECTORS.generatedImageImg;
      ...
      for (let attempt = 1; attempt <= 4; attempt++) {
        ...
        // 定位「目标图所属容器」内的下载按钮，并按需 scrollIntoView 到视口内
        const btnInfo = await op.query((targetIndex, imgSelsInner, btnSelsInner) => {
          const seen = new Set(); const imgs = [];
          for (const sel of imgSelsInner) {
            let found; try { found = document.querySelectorAll(sel); } catch { continue; }
            for (const el of found) { if (seen.has(el)) continue; seen.add(el);
              const r = el.getBoundingClientRect(); if (r.width < 80 && r.height < 80) continue; imgs.push(el); }
          }
          const i = targetIndex == null ? imgs.length - 1 : targetIndex;
          const img = imgs[i];
          if (!img) return { ok: false, error: 'img_not_found' };
          const container = img.closest('generated-image, .generated-image, [data-test-id*="generated-image"]') || img.parentElement;
          let btn = null;
          if (container) for (const sel of btnSelsInner) { try { btn = container.querySelector(sel); } catch { /* bad selector */ } if (btn) break; }
          if (!btn) return { ok: false, error: 'btn_not_in_container' };
          const vh = window.innerHeight;
          const before = btn.getBoundingClientRect();
          if (before.top < 60 || before.bottom > vh - 60) {
            btn.scrollIntoView({ behavior: 'instant', block: 'center' });
          }
          const r = btn.getBoundingClientRect();
          if (r.width === 0 || r.height === 0) return { ok: false, error: 'btn_not_visible' };
          const ir = img.getBoundingClientRect();
          return { ok: true, x: r.left + r.width / 2, y: r.top + r.height / 2,
                   hx: ir.left + ir.width / 2, hy: ir.top + ir.height / 2 };
        }, scrollResult.index, imgSels, btnSels);

        if (!btnInfo.ok) {
          console.log(`[downloadFullSizeImage] 第${attempt}次定位下载按钮失败(${btnInfo.error})，重试...`);
          continue;
        }

        // scrollIntoView 改变了滚动位置，重新 hover 图片新中心以维持容器 :hover（工具栏不消失），再点按钮
        await sleep(500);
        await page.mouse.move(btnInfo.hx, btnInfo.hy);
        await sleep(500);
        ...
        await sleep(60);
        await page.mouse.up();
        clickResult = { ok: true, selector: 'container-scoped download btn', x: btnInfo.x, y: btnInfo.y };
        break;
      }
      ...
      // 6. 等待下载完成（文件系统轮询，替代不可靠的 CDP 事件）
      //    setDownloadBehavior 在部分环境不会把文件重定向到 downloadDir，因此同时轮询
      //    downloadDir 与浏览器默认下载目录，取「点击后新出现、且大小已稳定」的文件，
      //    再统一移动到 downloadDir。
      try {
        const { join } = await import('node:path');
        const { statSync, existsSync, renameSync, copyFileSync } = await import('node:fs');

        const isTempDownload = f => /\.crdownload$/i.test(f)
          || /^\.com\.google\.Chrome\./.test(f)
          || /^未确认\s+\d+/.test(f)
          || /\.tmp$/i.test(f);
        ...
        while (Date.now() < deadline && !found) {
          ...
          for (const d of watchDirs) {
            ...
            const before = snapByDir.get(d) || new Set();
            const candidates = files.filter(f => !before.has(f) && !isTempDownload(f));
            if (!candidates.length) continue;
            // 取最近修改的候选作为下载目标
            let cand = null;
            for (const f of candidates) {
              let st; try { st = statSync(join(d, f)); } catch { continue; }
              if (!cand || st.mtimeMs > cand.mtimeMs) cand = { name: f, dir: d, size: st.size };
            }
            if (!cand) continue;
            // 大小连续两次相同且 >0，视为下载完成
            const s1 = cand.size;
            await sleep(800);
            let s2; try { s2 = statSync(join(cand.dir, cand.name)).size; } catch { continue; }
            if (s2 === s1 && s2 > 0) { found = cand; break; }
          }
        }

        if (!found) {
          console.warn('[ops] 下载超时：未检测到新文件');
          return { ok: false, error: 'download_timeout', src: imgInfo.src, index: imgInfo.index, total: imgInfo.total };
        }

        // 文件可能落在默认下载目录，统一移动到 downloadDir
        const targetName = found.name;
        const filePath = join(downloadDir, targetName);
        if (found.dir !== downloadDir) {
          try {
            renameSync(join(found.dir, found.name), filePath);
          } catch {
            copyFileSync(join(found.dir, found.name), filePath); // 跨盘 fallback
          }
        }
    ...
    async shareLatestImage({ index, timeout = 30_000, copyToClipboard = true, closeDialog = true } = {}) {
      ...
      for (let attempt = 1; attempt <= 5; attempt++) {
        ...
        shareBtnInfo = await op.query((targetIndex, selectors, imgSels) => {
          const _all = imgSels.flatMap(s => { try { return [...document.querySelectorAll(s)]; } catch { return []; } });
          const imgs = [...new Set(_all)].filter(el => { const r = el.getBoundingClientRect(); return r.width >= 80 || r.height >= 80; });
          const i = targetIndex == null ? imgs.length - 1 : targetIndex;
          const img = imgs[i];
          if (!img) return { ok: false, error: 'image_not_found' };

          const container = img.closest('.image-container, generated-image, .generated-image') || img.parentElement?.closest('.image-container, generated-image, .generated-image');
        ...
        }, imgInfo.index, SELECTORS.shareImageBtn, SELECTORS.generatedImageImg);
